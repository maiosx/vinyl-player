...
// Prefer a playing Spotify client, then any playing MPRIS player, then the
// first idle Spotify, then any idle player. The vinyl should keep turning
// for whatever is actually making sound.
function findPlayer(players) {
  var list = players && players.length !== undefined ? players : []
  var spotify = findSpotify(list)
...
function artProbeScript() {
  return [
    ...
    'dir="${XDG_CACHE_HOME:-$HOME/.cache}/vinyl-player/covers"',
    'mkdir -p "$dir" || exit 1',
